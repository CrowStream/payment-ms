...
public class GiftCardController {
    ...
    public ResponseEntity<Object> addGiftCard(@RequestBody GiftCard giftCard) {
        ...
        if (saved == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new MessageResponse("Invalid data for gift card creation"));
        }
    ...
    public ResponseEntity<Object> getGiftCardById(@PathVariable Long id) {
        ...
        if (giftCard == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new MessageResponse(String.format("Gift card with id %d does not exist", id)));
        }
    ...
    public ResponseEntity<Object> getGiftCard(@RequestParam(name = "card_code") String code) {
        ...
        if (giftCard == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new MessageResponse("No gift cards associated with the code provided"));
        }
        return ResponseEntity.status(HttpStatus.OK).body(giftCard);
    }
